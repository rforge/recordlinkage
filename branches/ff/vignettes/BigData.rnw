...
without blocking). In versions 0.3_x, this was achieved by blockwise on-demand
...
The interface to the \lqq{}big data\rqq methods has is compatible to code written
for version 0.3_x, so users familiar with these can stick to their existing
